...
export class AddBannedToUsers1787956769774 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX "public"."idx_saved_search_created_at"`,
    );
    await queryRunner.query(
      `ALTER TABLE "listings" DROP CONSTRAINT "listings_condition_check"`,
    );
    await queryRunner.query(
      `ALTER TABLE "listings" DROP CONSTRAINT "listings_annotation_level_check"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD "banned_at" TIMESTAMP WITH TIME ZONE`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "saved_searches" ALTER COLUMN "created_at" SET NOT NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "audit_log" ADD CONSTRAINT "CHK_61df26bb3e1e64395385850cb4" CHECK (action IN ('CREATE', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT', 'SOLD', 'WITHDRAWN','APPROVE_LISTING','REJECT_LISTING','BAN_USER'))`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD CONSTRAINT "FK_e10aa79b34032d2fce0a6c59176" FOREIGN KEY ("banned_by") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "notifications" ADD CONSTRAINT "FK_2b493a038e4fd0ea954648d6b44" FOREIGN KEY ("notification_from") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "notifications" ADD CONSTRAINT "FK_39dccc777268995087ce0ccd626" FOREIGN KEY ("entity_id") REFERENCES "listings"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }
