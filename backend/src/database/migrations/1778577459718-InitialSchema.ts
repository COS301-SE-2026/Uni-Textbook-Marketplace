...
export class InitialSchema1778577459718 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const modulesExistResult = (await queryRunner.query(`
      SELECT EXISTS (
    ...
    // Type-safe check with proper typing
    const modulesExist =
      modulesExistResult &&
      Array.isArray(modulesExistResult) &&
      modulesExistResult.length > 0 &&
      modulesExistResult[0]?.exists === true;
    ...
    if (!modulesExist) {
      ...
      const fkQueries: { query: string; name: string }[] = [
        {
      ...
      for (const { query, name } of fkQueries) {
        try {
          await queryRunner.query(query);
        } catch (err) {
          ...
          if (!error.message?.includes(`constraint "${name}" already exists`)) {
            throw error;
          }
        }
      }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const query of queries) {
      try {
        ...
        console.warn(`Failed to execute: ${query.substring(0, 60)}...`);
