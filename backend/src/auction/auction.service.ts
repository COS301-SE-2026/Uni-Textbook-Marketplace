...
export class AuctionService {
  ...
  async createAuction(
    ...
    if (listing.listing_status === ListingsStatus.SOLD) {
      throw new BadRequestException('Sold listings cannot be auctioned');
    }

    if (listing.listing_status === ListingsStatus.RESERVED) {
    ...
    if (existingAuction) {
      throw new BadRequestException(
        'This listing already has an active or scheduled auction',
      );
    }
    ...
    try {
      closeJob = await this.auctionQueue.add(
        'close-auction',
        {
  ...
  async placeBid(auctionId: string, bidderId: string, amount: number) {
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      auction.current_highest_bidder = {
        id: bidderId,
      } as Auction['current_highest_bidder'];
